...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey,           }, "Down", function()
        ...
        if client.focus then client.focus:raise() end
    end, {description = "focus client below", group = "focus"}),
    ...
    awful.key({ modkey, }, "e",
        function ()
            awful.spawn(terminal .. " --class vifm --title vifm --command vifm")
        end,
...
clientkeys = gears.table.join(
    ...
    -- Resizes the window to 50% of the screen. Mostly intended for use with floating windows
    awful.key({ modkey,           }, "-",  function (client)
        local screen = awful.screen.focused()
        local screen_width = screen.geometry.width
        local screen_height = screen.geometry.height

        client:geometry{width=screen_width / 2, height=screen_height / 2}
    end),
...
-- Mouse controls for selecting, moving and resizing clients
